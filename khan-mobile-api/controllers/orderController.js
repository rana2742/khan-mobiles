...
const courierPayload = (courier) => courier ? {
  ...
  statusCode: courier.statusCode,
  statusReason: courier.statusReason,
  slipLink: courier.slipLink,
  bookedAt: courier.bookedAt,
...
exports.myOrders = async (req, res) => {
  ...
  res.json({ success: true, orders: orders.map((o) => ({
    id: o._id.toString(), orderNumber: o.orderNumber, subtotal: Number(o.subtotal), discount: Number(o.discount), deliveryFee: Number(o.deliveryFee), total: Number(o.total), status: o.status,
    city: o.city, address: o.address, landmark: o.landmark, paymentMethod: o.paymentMethod, createdAt: o.createdAt, courier: courierPayload(o.courier),
    items: o.items.map((i) => ({ productId: i.product ? i.product.toString() : null, name: i.name, price: Number(i.price), quantity: i.quantity, imageUrl: i.imageUrl, alreadyReviewed: i.product ? reviewedSet.has(`${o._id}:${i.product}`) : true })),
  })) });
};
...
exports.cancelMine = async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (order.status !== 'pending') throw Object.assign(new Error(`This order is already ${order.status} and can no longer be cancelled. Contact us if you need help.`), { statusCode: 409 });
...
exports.stats = async (req, res) => {
  const [pendingCount, totalOrders, revenueAgg, lowStockCount, productCount] = await Promise.all([
    ...
    Product.countDocuments({ stock: { $lte: 10 }, isActive: true }), Product.countDocuments(),
  ]);
  res.json({ success: true, stats: { pendingCount, totalOrders, totalRevenue: Number(revenueAgg[0]?.total || 0), lowStockCount, productCount } });
};
